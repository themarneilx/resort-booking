...
export default function Navbar({ userName }: { userName?: string | null }) {
  ...
  const handleRoomsClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (pathname === "/") { // Only scroll if on the homepage
      e.preventDefault();
  ...
  const handleLogout = async () => {
    try {
      ...
      setAccessToken(null);
      window.location.reload(); // Refresh to update server session state
    } catch (error) {
    ...
    <>
      ...
      >
          <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
              ...
              <div className="hidden md:flex items-center space-x-8">
                  ...
                  <Link href="/rooms" onClick={handleRoomsClick} className="text-white/90 hover:text-white text-sm font-medium tracking-wide transition-colors">Rooms</Link>
      ...
      >
          ...
          <div className="flex-1 flex flex-col items-center justify-center space-y-8 p-6">
              ...
              <Link href="/rooms" onClick={handleRoomsClick} className="text-2xl font-serif text-white hover:text-brand-400 transition-colors">Rooms</Link>
              ...
              {userName ? (
                  <>
                    <p className="text-lg text-white font-bold">Hi, {userName}</p>
